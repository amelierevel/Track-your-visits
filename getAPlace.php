...
<div class="row">
    <div class="row">
            ...
            <div class="card">
                <div class="card-image">
                    <?php if (isset($picturePlace->picture)) { ?>
                        <img src="assets/img/placeImages/<?= $picturePlace->picture ?>" alt="Image représentant <?= $placeInfo->name ?>" title="Image de <?= $placeInfo->name ?>" class="responsive-img" />
                    <?php } else { ?>
                    ...
                    if (isset($_SESSION['isConnect'])) { //vérification que l'utilisateur est connecté 
                        if ($_SESSION['idUserTypes'] == 2) { //Affichage des fonctionnalités propres aux contributeurs
                            ?>
                        <a class="btn-floating halfway-fab waves-effect waves-light orange darken-3" id="addPictureFormButton" title="Ajout de photo"><i class="material-icons">add</i></a>
                            <?php
                        }
                    }
                    ?>
        ...
        <div class="col m7 s12">
            ...
            <?php if (isset($_SESSION['isConnect'])) { //si l'utilisateur est connecté affichage des icones ?>
                <div class="row right-align">
                    <div class="iconPlaceForUser">
                        <!--Formulaire ajout lieu à voir-->
    ...
    <?php if (isset($formError['picture'])) { //affichage du message d'erreur si le tableau d'erreur existe ?>
            ...
            <div class="row">
                <?php if (empty($timetablesList)) { ?>
                    ...
                    <table class="striped centered responsive-table col m10 offset-m1">
                        ...
                        <tbody>
                            <?php foreach ($timetablesList as $timetableDetail) { //boucle permettant d'afficher la liste des périodes horaires (timetableTypes) ?>
                                <tr>
                                    ...
                                    if (isset($_SESSION['isConnect'])) { //vérification que l'utilisateur est connecté
                                        if ($_SESSION['idUserTypes'] == 2) { //Affichage des fonctionnalités propres aux contributeurs
                                            ?>
                                            <td>
                                                <a href="Lieu?id=<?= $placeInfo->id ?>&idTimetableDelete=<?= $timetableDetail->id ?>" class="btn-floating btn-small waves-effect waves-light red accent-4 deleteButton" title="Suppression d'horaire"><i class="material-icons">delete</i></a>
                                                <?php if (isset($deleteTimetableError)) { //affichage du message d'erreur s'il existe ?>  
                            ...
                            <form action="#" method="POST" class="col s12 bgForm">
                                <div class="input-field col m4 s12">
                                    <select name="idDays">
                                        ...
                                        <?php foreach ($daysList as $dayDetail) { //boucle permettant d'afficher la liste des jours de la semaine ?>
                                    ...
                                    <?php if (isset($formError['idDays'])) { //affichage du message d'erreur si le tableau d'erreur existe ?>
                                ...
                                <div class="input-field col m4 s12">
                                    <select name="idTimetableTypes">
                                        ...
                                        <?php foreach ($timetableTypesList as $timetableTypeDetail) { //boucle permettant d'afficher la liste des périodes horaires (timetableTypes) ?>
                                    ...
                                    <?php if (isset($formError['idTimetableTypes'])) { //affichage du message d'erreur si le tableau d'erreur existe ?>
                                ...
                                <div class="input-field col m2 s5 offset-s1">
                                    ...
                                    <?php if (isset($formError['opening'])) { //affichage du message d'erreur si le tableau d'erreur existe ?>
                                ...
                                <div class="input-field col m2 s5">
                                    ...
                                    <?php if (isset($formError['closing'])) { //affichage du message d'erreur si le tableau d'erreur existe ?>
            ...
            <div class="row">
                <?php if (empty($pricesList)) { ?>
                    ...
                    <table class="striped centered responsive-table col m10 offset-m1">
                        ...
                        <tbody>
                            <?php foreach ($pricesList as $priceDetail) { //boucle permettant d'afficher la liste des périodes horaires (timetableTypes) ?>
                                <tr>
                                    ...
                                    if (isset($_SESSION['isConnect'])) { //vérification que l'utilisateur est connecté
                                        if ($_SESSION['idUserTypes'] == 2) { //Affichage des fonctionnalités propres aux contributeurs
                                            ?>
                                            <td>
                                                <a href="Lieu?id=<?= $placeInfo->id ?>&idPriceDelete=<?= $priceDetail->id ?>" class="btn-floating btn-small waves-effect waves-light red accent-4 deleteButton" title="Suppression tarif"><i class="material-icons">delete</i></a>
                                                <?php if (isset($deletePriceError)) { //affichage du message d'erreur s'il existe ?>  
                        ...
                        <div id="priceForm">
                            <form action="#" method="POST" class="col s12 bgForm">
                                <div class="input-field col m2 s12">
                                    <input type="text" name="price" id="price" placeholder="12.50€" class="validate" value="<?= isset($price->price) ? $price->price : '' ?>" />
                                    <label for="price">Tarif</label>
                                </div>
                                <div class="input-field col m4 s12">
                                    <select name="idPriceTypes">
                                        <option value="0" disabled selected>Type de tarif</option>
                                        <?php foreach ($priceTypesList as $priceTypeDetail) { //boucle permettant d'afficher la liste des jours de la semaine ?>
                                ...
                                <div class="input-field col m6 s12">
                                    <textarea class="materialize-textarea validate" name="priceName" id="priceName" data-length="100"><?= isset($place->name) ? $place->name : '' ?></textarea>

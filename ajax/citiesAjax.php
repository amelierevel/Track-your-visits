...
if (!empty($_POST['postalCode'])) {
    $city = NEW cities();
    //vérification de la validité de la valeur et attribution de cette valeur à l'attribut postalCode de l'objet $city avec la sécurité htmlspecialchars (évite injection de code)
    if (preg_match($regexPostalCode, $_POST['postalCode'])) {
        $city->postalCode = htmlspecialchars($_POST['postalCode']);
        $citiesResearch = $city->getCitiesListByPostalCode(); //appel de la méthode permettant l'affichage des villes
